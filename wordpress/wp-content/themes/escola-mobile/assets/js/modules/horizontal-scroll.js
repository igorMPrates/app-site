jQuery(document).ready(function ($) {
  ...
  function changeHistory(event) {
    ...
    if (event == "next") {
      ...
      atual >= 2020
        ? $(".next-history").addClass("opacity-0")
        : $(".next-history").removeClass("opacity-0");
      atual <= 1970
        ? $(".prev-history").addClass("opacity-0")
        : $(".prev-history").removeClass("opacity-0");
  ...
  function horizontalScroll(index) {
    if ($(".horizontal-scrolling-" + index + ".active").length > 0) {
      const theWidth =
        ($(".horizontal-scrolling-" + index + ".active")
          ...
          $(".horizontal-scrolling-" + index + ".active")
            ...
            .width()) *
        0.7;
      const item = $(".horizontal-scrolling-" + index + ".active").find(
        ".item:last-child"
      );
      const maxLeft = item.offset().left + item.width() + 80;
      $(".horizontal-scrolling-" + index + ".active").css(
        "height",
        Math.round(theWidth + 300) + "px"
      );
      ...
      $(window).on("scroll", function () {
        ...
        if (
          windowOffset > Elementoffset &&
          windowOffset < Elementoffset + (ElementHeight - $(window).height())
        ) {
          const atualHeight =
            windowOffset -
            $(".horizontal-scrolling-" + index + ".active").offset().top;
          const scrollPercent =
            (atualHeight * 100) / (ElementHeight - $(window).height());
          const useSize =
            (scrollPercent * (maxLeft - $(window).width() + 100)) / 100;
          $(
            ".horizontal-scrolling-" + index + ".active .horizontal-container"
          ).css("margin-left", `calc(-${useSize}px)`);
          $(
            ".horizontal-scrolling-" + index + ".active .horizontal-container"
          ...
          windowOffset >
          Elementoffset + (ElementHeight - $(window).height())
        ) {
          const maxSize = maxLeft - $(window).width() + 100;
          $(
            ".horizontal-scrolling-" + index + ".active .horizontal-container"
          ).css("margin-left", `calc(-${maxSize}px)`);
          $(
            ".horizontal-scrolling-" + index + ".active .horizontal-container"
          ).removeClass("fixed");
          $(
