...
<div class="bg-background overflow-x-hidden">
	<?php if ( ! empty( $_GET['s'] ) ) : ?>
		<div id="search-results" class="fixed right-0 top-0 h-screen w-full max-w-xl pt-136 z-40 bg-background">
			<div class="w-full h-full bg-background">

				<div class="mx-20 md:mx-48 pr-20 h-full overflow-x-hidden overflow-y-scroll search-scroll">

					<p class="text-sm font-bold">Resultados da busca por:</p>
					<p class="text-lg mb-20"><?php echo $_GET['s']; ?></p>
					...
					<?php if ( $the_query->have_posts() ) : ?>
						...
						while ( $the_query->have_posts() ) :
							...
							<a href="<?php the_permalink(); ?>">
								<div class="flex flex-col md:flex-row items-end md:items-center mb-20">
									<div class="w-200 md:w-180 h-200 md:h-180 min-w-180 mr-20 rounded-lg bg-primary-100 bg-cover bg-center" style="background-image: url('<?php the_post_thumbnail_url(); ?>');"></div>
									<div class="w-180 md:w-full mr-20">
										<h3 class="text-lg mt-10 md:mt-0 mb-10 md:mb-15"><?php echo get_the_title(); ?></h3>
										<p class="text-xs"><?php echo get_the_excerpt(); ?></p>
									</div>
								</div>
							</a>
					...
					<?php else : ?>
						<p><?php _e( 'Desculpa, não há resultados correspondentes a sua pesquisa!' ); ?></p>
					<?php endif; ?>

				</div>
			</div>
		</div>
		<div class="fixed w-full h-full z-30 bg-white opacity-50"></div>
	<?php endif; ?>

	<div class="h-136"></div>

	<header id="main-header" class="fixed left-0 top-0 w-full slide section fp-auto-height z-50">
		<div class="header-wrapper bg-opacity-75 bg-background h-136 relative flex items-center duration-300 z-50">
			<div class="container mx-auto flex items-center justify-between px-20">
				<div class="main-logo flex items-center">
					<a href="<?php echo get_home_url(); ?>"><img class="h-40 lg:h-64 w-auto duration-300" src="<?php echo get_template_directory_uri(); ?>/dist/images/logo.svg" alt="escola móbile"></a>
				</div>
				<div class="flex items-center justify-end">
					...
					<div class="relative z-40">
						<div class="flex items-center">
							...
							<form class="search-area bg-dark h-48 px-13 flex items-center justify-center rounded-full cursor-pointer" method="get">
								<input name="s" class="search-item duration-300 bg-transparent outline-none focus:outline-none text-white w-0" type="text" placeholder="Pesquisar...">
								<button class="outline-none focus:outline-none" type="submit">
									<svg class="search-button" width="23" height="23" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
										<path d="M10.3889 19.2778C15.2981 19.2778 19.2778 15.2981 19.2778 10.3889C19.2778 5.47969 15.2981 1.5 10.3889 1.5C5.47969 1.5 1.5 5.47969 1.5 10.3889C1.5 15.2981 5.47969 19.2778 10.3889 19.2778Z" stroke="#F5F5F1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
										<path d="M21.4993 21.4998L16.666 16.6665" stroke="#F5F5F1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
									</svg>
								</button>
							</form>
							...
							<?php else : ?>
								<a href="/" class="close-search border-none mr-20 outline-none focus:outline-none duration-300">
									<svg xmlns="http://www.w3.org/2000/svg" class="h-64 w-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
										<path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
									</svg>
								</a>
							<?php endif; ?>

							<a href="https://exclusiva.escolamobile.com.br/login.php" target="_BLANK" class="bg-dark hover:bg-green-100 duration-300 h-48 px-16 hidden xl:flex items-center justify-center rounded-full cursor-pointer text-white">
								Área Exclusiva
							</a>

							<div class="relative h-48 w-64 hidden">
								<ul class="language-select absolute top-0 left-0 bg-white text-dark py-13 px-16 flex items-start justify-center flex-col rounded-base cursor-pointer text-white">
									<li class="flex items-center group underline">
										<div>PT</div>
										<svg class="ml-5" width="13" height="7" viewBox="0 0 13 7" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M11.8224 0.968286L6.95238 5.78363C6.61749 6.11477 6.20488 6.11477 5.86998 5.78363L0.999978 0.968283" stroke="#2C2C2C" stroke-width="1.5" />
										</svg>
									</li>
									<li class="disable-language">EN</li>
								</ul>
							</div>
						</div>
					</div>
		...
		<?php if ( is_page( 'nossa-historia' ) ) : ?>
			...
			<div class="px-30 mt-10 xl:hidden">
				<div class="w-full mt-20 py-6 px-20 flex justify-between items-center bg-dark text-white rounded-full">
					<button class="prev-history focus:outline-none opacity-0">
						<svg class="transform rotate-180" width="23" height="24" viewBox="0 0 23 24" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M11.811 1.56531L21.0968 10.9565C21.7353 11.6023 21.7353 12.3979 21.0968 13.0437L11.811 22.4349" stroke="#F5F5F1" stroke-width="2.11826" />
							<line x1="21.5757" y1="12.0658" x2="0.131725" y2="12.0658" stroke="#F5F5F1" stroke-width="2.11826" />
						</svg>
					</button>
					<a href="#1970" class="history-link flex items-center">
						<div id="history-color" class="h-20 w-20 rounded-full bg-pink-100 mr-10"></div>
						<span id="history-active" class="text-xl font-semibold">1970</span>
					</a>
					<button class="next-history focus:outline-none">
			...
			<div class="h-10 w-full bg-dark items-center justify-between relative z-40 px-80 hidden xl:flex">
				<div id="to-1970" class="time-navigation bg-pink-100">
					<div class="time-span relative">
						<div class="relative z-10">1970</div>
						<svg class="absolute top-0 left-0 ml-25 -mt-5" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="11.0234" y="-1" width="17" height="17" rx="4" transform="rotate(45 11.0234 -1)" fill="#2C2C2C" />
						</svg>
					</div>
				</div>
				<div id="to-1980" class="time-navigation bg-yellow">
					<div class="time-span relative">
						<div class="relative z-10">1980</div>
						<svg class="absolute top-0 left-0 ml-25 -mt-5" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="11.0234" y="-1" width="17" height="17" rx="4" transform="rotate(45 11.0234 -1)" fill="#2C2C2C" />
						</svg>
					</div>
				</div>
				<div id="to-1990" class="time-navigation bg-primary-100">
					<div class="time-span relative">
						<div class="relative z-10">1990</div>
						<svg class="absolute top-0 left-0 ml-25 -mt-5" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="11.0234" y="-1" width="17" height="17" rx="4" transform="rotate(45 11.0234 -1)" fill="#2C2C2C" />
						</svg>
					</div>
				</div>
				<div id="to-2000" class="time-navigation bg-white">
					<div class="time-span relative">
						<div class="relative z-10">2000</div>
						<svg class="absolute top-0 left-0 ml-25 -mt-5" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="11.0234" y="-1" width="17" height="17" rx="4" transform="rotate(45 11.0234 -1)" fill="#2C2C2C" />
						</svg>
					</div>
				</div>
				<div id="to-2010" class="time-navigation bg-blue-400">
					<div class="time-span relative">
						<div class="relative z-10">2010</div>
						<svg class="absolute top-0 left-0 ml-25 -mt-5" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="11.0234" y="-1" width="17" height="17" rx="4" transform="rotate(45 11.0234 -1)" fill="#2C2C2C" />
						</svg>
					</div>
				</div>
				<div id="to-2020" class="time-navigation bg-orange">
					<div class="time-span relative">
						<div class="relative z-10">2020</div>
						<svg class="absolute top-0 left-0 ml-25 -mt-5" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect x="11.0234" y="-1" width="17" height="17" rx="4" transform="rotate(45 11.0234 -1)" fill="#2C2C2C" />
						</svg>
					</div>
		...
		<div class="mobile-menu fixed w-screen h-screen overflow-y-auto overflow-x-hidden max-w-md left-0 top-0 bg-background xl:hidden duration-300 pt-96 px-30 z-40 -ml-screen">
			...
			<div class="max-h-screen pb-30 flex flex-col items-start">
				...
				wp_nav_menu(
					array(
						'theme_location' => 'primary',
						'menu_class'     => 'text-2xl mr-15 flex flex-col items-start space-y-10 mt-5',
					)
				);
				?>

				<div class="my-20">
					<a href="https://exclusiva.escolamobile.com.br/login.php" target="_BLANK" class="bg-dark py-15 text-white px-20 rounded-full hover:bg-green-100 duration-300">
						Área Exclusiva
					</a>
				</div>
